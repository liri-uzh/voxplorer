...
class TestFeatureExtractor(unittest.TestCase):
    def setUp(self):
        ...
        self.audio_files = [os.path.join(os.path.dirname(
            __file__), 'data', f) for f in os.listdir(self.audio_path)]
        ...
        self.audio_librosa, self.sr_librosa = librosa.load(
            self.audio_files[0], sr=None)
        ...
        self.feature_extractor = FeatureExtractor(
            audio_path=self.audio_path, feature_methods=self.feature_methods, metavars=self.metavars)
    ...
    def test_speaker_embeddings(self):
        speaker_embeddings = self.feature_extractor.speaker_embeddings(
            self.audio_torch)
        self.assertEqual(speaker_embeddings.shape, (1, 192))

    def test_extract_metadata(self):
        expected = {'filename': [os.path.basename(
            self.audio_files[0])], 'speaker': [os.path.basename(self.audio_files[0]).split('_')[0]]}
        metadata = self.feature_extractor.extract_metadata(
            self.audio_files[0], **self.metavars)
        assert dict(metadata) == expected

    def test_process_files(self):
        expected_metadata = {'filename': [os.path.basename(f) for f in self.audio_files], 'speaker': [
            os.path.basename(f).split('_')[0] for f in self.audio_files]}
        expected_feature_labels = [f'mfcc{i+1}_mean' for i in range(10)] + \
